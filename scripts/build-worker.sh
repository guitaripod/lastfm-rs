...
#
# worker 0.6.x locks wasm-bindgen to 0.2.104 (via js-sys 0.3.81), but the pinned
# worker-build downloads the 0.2.105 CLI, whose bindgen schema is incompatible.
# We seed worker-build's cache with a matching 0.2.104 CLI so the build is
# reproducible across local, CI and self-host environments.
